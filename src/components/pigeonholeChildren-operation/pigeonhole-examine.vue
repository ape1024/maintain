...
<script>
import { maintainDailyapprovalTaskDetail, getAllDetailsByDeviceId, maintainDailygetEquirementjudgments2, maintainDailygetTaskApprovalItems, deleteTaskDetail, getTaskParams, findAllApproval } from '../../api/user'
import DialogImg from 'base/dialog-img/dialog-img'
import $ from 'jquery'
export default {
  name: 'pigeonhole-examine',
  ...
  methods: {
    ...
    DetailsByDeviceId () {
      this.axios.post(getAllDetailsByDeviceId(this.taskidCode, this.equipmentCode)).then((response) => {
        if (response.data.code === 0) {
          let arrData = []
